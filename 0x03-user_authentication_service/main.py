...
def register_user(email: str, password: str) -> None:
    ...
    Returns:
        None
    """
    r = requests.post('http://127.0.0.1:5001/users',
...
def log_in_wrong_password(email: str, password: str) -> None:
    ...
    Returns:
        None
    """
    r = requests.post('http://127.0.0.1:5001/sessions',
                      data={'email': email, 'password': password})
...
def profile_unlogged() -> None:
    ...
    Returns:
        None
    """
    r = requests.get('http://127.0.0.1:5001/profile')
...
def log_out(session_id: str) -> None:
    ...
    """
    cookies = {'session_id': session_id}
    r = requests.delete('http://127.0.0.1:5001/sessions',
                        cookies=cookies)
    if r.status_code == 302:
        assert(r.url == 'http://127.0.0.1:5001/')
    else:
        assert(r.status_code == 200)
...
def reset_password_token(email: str) -> str:
    ...
    """
    r = requests.post('http://127.0.0.1:5001/reset_password',
                      data={'email': email})
    if r.status_code == 200:
...
def update_password(email: str, reset_token: str,
    ...
    r = requests.put('http://127.0.0.1:5001/reset_password',
                     data=data)
    if r.status_code == 200:
